...
class Runner: Zombie {
    ...
    init() {
        ...
        let config = zombieConfigs[ZombieType.runner.rawValue] ?? [:]
        ...
        let texture = ResourceManager.shared.getTexture(named: "runner_move_1")
    ...
    private func setupMoveAnimation() {
        ...
        if let animation = ResourceManager.shared.createAnimation(forKey: "runner_move", timePerFrame: 0.2, repeatForever: true) {
            // 保存移动动画
            ...
            print("警告：无法从ResourceManager获取runner_move动画，创建备用动画")
    ...
    private func setupAttackAnimation() {
        ...
        let attackFrameNames = ["runner_attack_1", "runner_attack_2"]
