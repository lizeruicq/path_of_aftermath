...
class ShotGun: Defend {
    ...
    init() {
        ...
        let texture = ResourceManager.shared.getTexture(named: "shotgun_idle")
    ...
    override func attackTarget(_ target: Zombie) {
        ...
        // 子弹间隔距离
        let bulletSpacing: CGFloat = 10.0

        // 发射三颗子弹：左、中、右
        let bulletOffsets: [CGFloat] = [-bulletSpacing, 0, bulletSpacing]

        for offset in bulletOffsets {
            createAndFireBullet(at: towerPositionInScene, xOffset: offset, scene: scene)
        }
    }

    // 创建并发射单颗子弹
    private func createAndFireBullet(at towerPosition: CGPoint, xOffset: CGFloat, scene: SKScene) {
        // 创建子弹
        ...
        // 设置子弹初始位置在炮塔上方一点，加上X轴偏移
        bullet.position = CGPoint(x: towerPosition.x + 13 + xOffset, y: towerPosition.y + 20)
        ...
        // 计算目标位置（直线向上，保持X轴偏移）
        let targetY = towerPosition.y + distance
        let targetPosition = CGPoint(x: towerPosition.x + xOffset, y: targetY)
    ...
    private func playShootSound() {
        // 创建射击音效动作
        let soundAction = SKAction.playSoundFileNamed("shotgun_shot.mp3", waitForCompletion: false)
    ...
    override func stopAttackingAnimation() {
        ...
        // 恢复原始纹理
        self.texture = ResourceManager.shared.getTexture(named: "shotgun_idle")
    }

    // 播放射击动画
    private func playShootAnimation() {
        // 从ResourceManager获取动画
        if let animation = ResourceManager.shared.createAnimation(forKey: "shotgun_shoot", timePerFrame: 0.05) {
            // 播放射击动画
            self.run(animation, withKey: "shootAnimation")
        } else {
            // 如果从ResourceManager获取失败，创建备用动画
            print("警告：无法从ResourceManager获取shotgun_shoot动画，创建备用动画")
